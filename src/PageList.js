...
export default class PageList extends React.PureComponent {
  ...
  handleWaypointEnter = (page) => {
    ...
    if (page && page.pageIndex != 0 && page.pageIndex + 1 == this.props.last) {
      this.props.setFirst(page.pageIndex - 1);
      this.props.setLast(page.pageIndex + 9);
    }
    console.log(this.props.first, this.props.last);
    if (page && page.pageIndex != 0 && page.pageIndex == this.props.first) {
      this.setState({
        number: page.pageIndex + 1,
        last: this.props.first > 9 ? page.pageIndex - 9 : 0
      });
      this.props.setFirst(this.state.last > 9 ? page.pageIndex - 9 : 0);
      this.props.setLast(page.pageIndex + 1);
    }
  };
  renderPage = ({
    index, // Index of row
  ...
  calculateScale = (page) => {
    ...
    const rotated = page.pageInfo.rotate === 90 || page.pageInfo.rotate === 270;
    ...
    return (
      <div className="page-list">
        {[...Array(this.props.pageCount).keys()]
          .slice(this.props.first, this.props.last)
          .map((index) =>
            this.renderPage({
              index,
              key: `page-${index}`
